    ...
    <div className="grid grid-cols-3 gap-2.5 px-2">
      {options.map(o => (
        <button key={o.id} onClick={() => onSelect(o.id)} aria-label={o.label}
          ...
          <span className={`relative block aspect-square overflow-hidden rounded-2xl border-2 transition ${selected === o.id ? 'border-[var(--ink)]' : 'border-transparent group-hover:border-black/15'}`}>
