...
function Silhouette({ id }: { id: string }) {
  return (
    <svg viewBox="0 0 64 64" width="56" height="56" fill="var(--ink)" aria-hidden="true">
      ...
      {id === 'neutro' && (
        <><rect x="23" y="24" width="18" height="24" rx="9" /><rect x="25" y="46" width="6" height="14" rx="3" /><rect x="33" y="46" width="6" height="14" rx="3" /></>
      )}
      {id !== 'mujer' && id !== 'neutro' && (
...
export function GenderChoice({ options, selected, onSelect }: {
  ...
  return (
    <div className="mx-auto grid max-w-sm grid-cols-3 gap-3 md:gap-4">
      {options.map(o => (
        <button key={o.id} onClick={() => onSelect(o.id)} aria-label={o.label}
          className={`flex w-full flex-col items-center gap-3 rounded-3xl border-2 px-2 py-6 transition ${selected === o.id ? 'border-[var(--ink)] bg-[var(--banana)]/15' : 'border-black/10 hover:border-black/25'}`}>
          <Silhouette id={o.id} />
